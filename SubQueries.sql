...
-- Question1: Retrieve the names of employees who earn more than the average salary of all employees.
SELECT name, salary
FROM employee
...
-- INTERMEDIATE LEVEL QUestion1: Find employees who work in the same department as Abhishek.
SELECT name
FROM employee
WHERE department_id = (SELECT department_id FROM employee WHERE name ='Abhishek');

-- Question2: List departments that have at least one employee earning more than ₹60,000.
SELECT DISTINCT department_name
FROM department
WHERE department_id IN (SELECT department_id FROM employee WHERE salary>60000);

-- Question3: Find the department name of the employee who made the highest sale.
...
-- Question4: Retrieve employees who have made sales greater than the average sale amount.
SELECT name
FROM employee
...
-- Question5: Find the total sales made by employees who earn more than the average salary
...
-- ADVANCE LEVEL Question1: Find employees who have not made any sales.
SELECT *
FROM employee
...
-- Question2: List departments where the average salary is above ₹55,000.
SELECT department_name
FROM department
...
-- QUEStion3: Retrieve department names where the total sales exceed ₹10,000.
SELECT department_name
FROM department
WHERE department_id IN (SELECT department_id 
...
-- Question4: Find the employee who has made the second-highest sale.
SELECT name 
FROM employee
WHERE emp_id = (SELECT emp_id
FROM sales
...
-- Question5: Retrieve the names of employees whose salary is greater than the highest sale amount recorded.
SELECT name
FROM employee
